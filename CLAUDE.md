...
- **Question/limb data model**: `questions[]` → each has `subject`/`category`/`source`/`questionText`/`limbs[]`. A limb can be a plain O/X limb, a multi-choice limb (`options[]` + `correctText`), a free-text limb (`acceptedAnswers[]`), or an inline O/X limb (「（①語句）〇×」 markers parsed out of `text` by `parseInlineOxItems`/`getInlineOxExpectedAnswers` and rendered as individually-clickable spans by `renderInlineOxText`). `getAllLimbs()` flattens all questions into a single limb list for study sessions and stats, with an option to split inline-O/X limbs into individually-tracked pseudo-records (`makeInlineRecordId`).
  - Each inline-O/X blank is answered/recorded independently under its own child record id (`` `${limbId}::${key}` ``, via `makeInlineRecordId`) — the parent limb's own `records[limbId]` is **never written**. Any code that needs "how has this limb actually performed" (study-mode filters in `startSession`, `updateMasteryCounts`, per-limb rate display) must go through `getEffectiveRecord(limb)`, not `getRecord(limb.id)`, or inline-O/X limbs silently vanish from 苦手/まちがえたもの/復習 modes and their stats always read as unanswered. `getEffectiveRecord` aggregates the child records (sum `correct`/`wrong`, union `wrongDateKeys`, `lastWrong` true if any child's last answer was wrong, `review` merged min/max across children) for a bare parent limb id, and simply delegates to `getRecord` for ordinary limbs and for already-split child ids (`id.includes('::')`, as produced by `getAllLimbs(..., true)` for the stats page). `weakScore`/`reviewPriorityScore`/`isDueForReview` take a limb object (not a bare id) for this reason.
